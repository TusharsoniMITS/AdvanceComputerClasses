...
function EditHindiText() {
    ...
    const fetchTexts = async () => {
        try {
            const res = await axios.get('/api/getAllHindiText')
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`/api/DeleteHindiText/${id}`)
            fetchTexts()
        } catch (err) {
            console.log(err)
        }
    ...
    return (
        <div style={{ padding: '20px' }}>
            <h2 style={{ textAlign: 'center' }}>Edit Hindi Text</h2>
